...
{
    public partial class ListAndGraphForSelectedDate
    {
        ...
        DateTime? selectedDate { get; set; } = DateTime.Now.Date;
        ...
        {
            if (modules.Any(i=>i == selectedModule))
            {
                disableSubmit = false;
            }
            else
        ...
        public async void ListValues()
        {
            if (selectedModule.Id != 0)
            {
                ...
                _Mq8Series.Clear();
                _Mq9Series.Clear();
                valuesForSelection = await _SensorService.GetValuesForSelectedDateByModuleId(selectedModule.Id, (DateTime)selectedDate);
